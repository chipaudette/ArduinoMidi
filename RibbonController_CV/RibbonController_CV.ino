/*
 Created: Chip Audette, Apr 2016
 Purpose: Sense Reisitive Ribbon and Output MIDI/CV notes
 ...
 Physical Setup, if using MIDI:
     Using old Sparkfun MIDI shield
     Using Ribbon connected to A3 (wiper) and Gnd

 Physical Setup, if using DAC for CV:   DAC = MCP4922 12-bit 2-channel DAC
   MCP4922 Pin3 = (NOT)CS   = Arduino Micro Pin 10  (SS)
   MCP4922 Pin4 = SCK       = Arduino Micro Pin SCK (SCK)
   MCP4922 Pin5 = SDI       = Arduino Micro Pin MO  (MOSI)
   MCP4922 Pin8 = (NOT)LDAC = LOW
   MCP4922 Pin9 = (NOT)SHDN = HIGH
   MCP4922 Pin11 = Pin12 = Vref = HIGH
   
   MCP4922 Pin14 = Vout A...Pitch CV, Volt/Oct
   MCP4922 Pin10 = Vout B...Trigger, LOW means Note On
...
const int LED0_pin = 13; //on Arduino Board
...
#define MIDI_NOTE_ON_VEL (127)  //0x7F
#define MIDI_NOTE_OFF_VEL (0)  //0x7F
#define MIDI_PITCH_BEND (0xE0+MIDI_CHANNEL)
#define MIDI_CC (0b10110000 + MIDI_CHANNEL)
#define MIDI_BRIGHTNESS_CC ((byte)74)
...
const float DAC_counts_per_half_step = (((float)MAX_DAC_COUNTS/MAX_DAC_VOLT)*VOLT_PER_OCTAVE / 12.0);
...
void setup() {
  ...
  //initialize the DAC
  pinMode(trig_pin,OUTPUT); digitalWrite(trig_pin,TRIG_NOTE_OFF); 
  ...
  //setup the LEDs
  pinMode(LED0_pin, OUTPUT); digitalWrite(LED0_pin, LOW); //LOW is off for he Arduino LED
...
int raw_ribbon_value = 0, ribbon_value = 0;
float ribbon_value_float = 1023.0, prev_ribbon_val_float = 1023.0;
int closest_note_num = 0, prev_closest_note_num = 0;
float note_num_float = 0.0, prev_note_num_float = 0.0;
int note_num = 0, prev_note_num = 0;
int MIDI_note_num = 0, prev_MIDI_note_num = 0;
boolean is_note_on = false, prev_is_note_on = false;
boolean note_change;
...
byte pitch_bend_LSB, prev_pitch_bend_MSB = 63;
byte pitch_bend_MSB, prev_pitch_bend_LSB = 63;
...
void updateTheSystem() {
  ...
  if (ribbon_value < (RIBBON_SPAN-10)) {
    if ((note_num >= 0) && (note_num <= ribbon_span_half_steps_float)) is_note_on = true;
    if (abs(note_num - prev_note_num) > max(max_allowed_jump_steps,synth_bend_half_steps)) is_note_on = false;
  } else {
  ...
  boolean note_change = (is_note_on != prev_is_note_on) | (note_num != prev_note_num);
  ...
  //light LEDs if note is on
  digitalWrite(LED0_pin, is_note_on);
  digitalWrite(LED1_pin, !is_note_on); //if note is on (ie HIGH) turn on the LED (LOW for the Sparkfun shield)
  ...
  if (note_change) {
    ...
    if (PRINT_TEXT) {
      ...
      if (is_note_on) {
        ...
        //turn on new note
        Serial_t.write((byte)MIDI_NOTE_ON);
        Serial_t.write((byte)MIDI_note_num);
        Serial_t.write((byte)MIDI_NOTE_ON_VEL);  //velocity
        ...
        if (prev_is_note_on == true) {
          //previous note was on...so assume legato...turn off old note now that new one is already started
          Serial_t.write((byte)MIDI_NOTE_OFF);
          Serial_t.write((byte)prev_MIDI_note_num);
          Serial_t.write((byte)MIDI_NOTE_OFF_VEL);
        }
        #endif

        #if WRITE_DAC
        updateCV(note_num_float,is_note_on);
        #endif
        
      } else if ((is_note_on == false) && (prev_is_note_on == true)) {
        ...
        #if WRITE_DAC
        note_num_float = prev_note_num_float;
        updateCV(note_num_float,is_note_on);
        #endif
      }
    //}
...
void transmitBrightness(byte value) {
  ...
  Serial_t.write((byte)MIDI_BRIGHTNESS_CC);
...
int updateCV(const float &note_num_float,const boolean &is_note_on){
  int trig_val_counts = TRIG_NOTE_ON;
  if (!is_note_on) trig_val_counts = TRIG_NOTE_OFF;
  
  int pitch_val_counts = (int)((note_num_float+CV_note_bottom) * DAC_counts_per_half_step);
  pitch_val_counts = constrain(pitch_val_counts,0,MAX_DAC_COUNTS);

  const float KBD_track = 0.6;
  int filter_val_counts = (int)(note_num_float * DAC_counts_per_half_step * KBD_track);
  filter_val_counts = constrain(filter_val_counts,0,MAX_DAC_COUNTS);
  
  if (PRINT_CV_TEXT) {
    Serial.print("DAC: is_note_on = "); Serial.print(is_note_on);
    Serial.print(", pitch = "); Serial.print(pitch_val_counts);
    Serial.print(", trig = "); Serial.print(trig_val_counts);
    Serial.println();
  }
  if (false) {
    //write trigger on/off from the DAC chan #2
    MCP4922_write(slaveSelectPin,pitch_val_counts,trig_val_counts);
  } else {
    //write trigger from digital pin and write filter command from the DAC chan #2
    MCP4922_write(slaveSelectPin,pitch_val_counts,filter_val_counts);
    digitalWrite(trig_pin,constrain(trig_val_counts,LOW,HIGH));
  }


  return pitch_val_counts;
}
